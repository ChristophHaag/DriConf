...
class OptionLine:
    ...
    def __init__(self, page, i, opt, simple, removable=False):
        ...
        self.label.show()
        page.table.attach(self.label, 0, 1, i, i + 1, gtk.EXPAND | gtk.FILL, 0,
                          5, 5)
        ...
        if removable:
            ...
            page.tooltips.set_tip(self.resetButton,
    ...
    def doValidate(self):
        ...
        if (valid and not self.isValid) or (not valid and self.isValid):
            ...
            self.widget.show()
            self.page.table.attach(self.widget, 1, 2, i, i + 1, gtk.FILL, 0, 5,
                                   5)
            # Update parent page.
            self.page.doValidate()
...
class UnknownSectionPage(gtk.VBox):
    ...
    def help(self, widget):
        if self.driver:
            ...
            msg = _("The driver for this device could not be determined or "
                    ...
                    "supported and valid.")
        dialog = gtk.MessageDialog(mainWindow, gtk.DIALOG_DESTROY_WITH_PARENT,
    ...
    def editedSignal(self, widget, row, newVal, value):
        ...
        for i in range(row):
            cursor = self.store.iter_next(cursor)
        if value:
            if self.app.options[name] == newVal:
                return
            self.app.options[name] = newVal
            self.store.set(cursor, 0, str(name), 1, str(newVal), 2,
                           self.nameEditable, 3, self.valueEditable)
        else:
            ...
            self.opts[row] = name
            self.app.options[name] = val
            self.store.set(cursor, 0, str(name), 1, str(val), 2,
                           self.nameEditable, 3, self.valueEditable)
        self.app.modified(self.app)
...
if gtk.__dict__.has_key("AboutDialog"):
    ...
    class AboutDialog(gtk.MessageDialog):
        def __init__(self):
            ...
            if translators:
                text = text + (u"\n\n%s: %s" %
                               (_("Translated by"), _("translator-credits")))
            gtk.MessageDialog.__init__(
                self, mainWindow, gtk.DIALOG_DESTROY_WITH_PARENT
                | gtk.DIALOG_MODAL, gtk.MESSAGE_INFO, gtk.BUTTONS_CLOSE, text)
