    ...
    def validate(self):
        ...
        for sectPage in self.sectPages:
            ...
            if not valid:
                ...
                self.sectLabels[index].modify_fg(gtk.STATE_NORMAL,
                                                 gtk.gdk.Color(65535, 0, 0))
                self.sectLabels[index].modify_fg(gtk.STATE_ACTIVE,
                                                 gtk.gdk.Color(65535, 0, 0))
            else:
                self.sectLabels[index].modify_fg(gtk.STATE_NORMAL,
                                                 self.default_normal_fg)
                self.sectLabels[index].modify_fg(gtk.STATE_ACTIVE,
                                                 self.default_active_fg)
...
class ConfigTreeModel(gtk.GenericTreeModel):
    ...
    def renderIcons(self, widget):
        ...
        self.appIcon = widget.render_icon("gtk-execute", gtk.ICON_SIZE_MENU,
                                          None)
        self.unspecIcon = widget.render_icon("gtk-dialog-question",
                                             gtk.ICON_SIZE_MENU, None)
        missing = widget.render_icon("gtk-missing-image", gtk.ICON_SIZE_MENU,
                                     None)
        ...
        self.screenDriverIcon = self.iconFromShared("screencard.png", size,
                                                    missing)
    ...
    def on_get_path(self, node):
        if node.__class__ == dri.DRIConfig:
            ...
            config = node.config
            return (self.configList.index(config), config.devices.index(node))
        elif node.__class__ == dri.AppConfig:
            device = node.device
            config = device.config
            return (self.configList.index(config),
                    config.devices.index(device), device.apps.index(node))
        else:
            assert 0
    ...
    def nodeModified(self, node, b=True):
        ...
        if config.isModified != b:
            config.isModified = b
            curNode = commonui.mainWindow.configTree.getSelection(
                allowNone=True)
            if not curNode:
...
class ConfigTreeView(gtk.TreeView):
    def __init__(self, configList):
        ...
        self.connect("style-set",
                     lambda widget, prevStyle: self.model.renderIcons(widget))
